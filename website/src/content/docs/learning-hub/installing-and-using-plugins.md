...
*(v1.0.79+)* Add `"autoUpdate": true` to an `extraKnownMarketplaces` entry to automatically refresh that marketplace's plugins at session start:
...
This is useful for internal marketplaces where you always want the latest plugin versions without running `copilot plugin marketplace update` manually.
