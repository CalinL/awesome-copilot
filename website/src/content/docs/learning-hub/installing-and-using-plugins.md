...
| **Canvas Extensions** | Copilot app canvas extensions shipped inside a plugin (v1.0.79+) | `com.github.copilot/extensions/` |
...
*(v1.0.79+)* **Auto-updating marketplace plugins**: Set `"autoUpdate": true` on an `extraKnownMarketplaces` entry to automatically update its plugins at session start — useful for internal marketplaces where you always want the latest version:
