...
The `/worktree` command (v1.0.61+) creates a **new git worktree** and switches into it, leaving your uncommitted changes behind in the original session. Use `/move` (v1.0.72+) when you want to carry uncommitted changes *into* the new worktree instead. This split keeps both operations clear and intentional:
...
After the command runs, the session is inside the new worktree. Use `/worktree` when you want to start fresh on a parallel branch, and `/move` when you want to bring your in-progress work with you. In v1.0.64+ you can also use the experimental `--worktree` flag at startup (`copilot -w [name]`) to create or reuse a worktree under `<repo>.worktrees/` before the session begins.
