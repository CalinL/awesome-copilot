...
| `defaultMode` | Choose the startup mode for new interactive sessions (`interactive`, `plan`, `autopilot`) (v1.0.81+) |
| `defaultPermissionMode` | Set the default approval behavior for new interactive sessions (v1.0.81+) |
...
**Session restore on startup** (v1.0.81+): If the CLI exits unexpectedly — due to a crash or machine restart — the next startup automatically offers to restore sessions that were still open. This means you won't lose work-in-progress sessions when your terminal is closed abruptly.
...
**Voice dictation** (v1.0.81+): Press **Ctrl+Space** to toggle voice dictation. When active, your spoken words are transcribed directly into the prompt input, letting you describe tasks hands-free without typing.
...
The `--add-dir` flag *(v1.0.81+)* adds an extra directory from which Copilot discovers custom agents and skills. This lets you share a central library of agents and skills across multiple projects without installing a plugin:

```bash
copilot --add-dir ~/my-shared-agents
```

Any `.agent.md` files and skill folders in the specified directory are loaded alongside your project's `.github/` resources.
