...
Yes — as of v1.0.74, **multi-turn subagents are always enabled**. You can send follow-up messages to a subagent that is currently running, allowing you to steer or clarify its task in real time. Previously this required opting in; it is now on by default for all sessions.
