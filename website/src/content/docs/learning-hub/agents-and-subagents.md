...
The default maximum nesting depth is **4** (lowered from 6 in v1.0.72). Usage-based billing users can raise this up to 128 by setting `subagents.maxDepth` in `/settings`.

**Can I send follow-up messages to a running subagent?**

Yes. Since v1.0.72, **multi-turn subagents are always enabled** — you can send follow-up messages to a subagent while it is still running without any additional configuration. This replaces the earlier opt-in toggle and makes interactive subagent iteration available by default.
