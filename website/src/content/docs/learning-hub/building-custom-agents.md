...
| Fast, efficient reasoning | grok-4.5 *(v1.0.76+)* |
| Quick analysis, simple tasks | Claude Haiku or GPT-4.1-mini |
| Large codebase understanding | Models with larger context windows |

### Setting a Model for Plan Mode

When working in plan mode, you can choose a separate model specifically for the planning phase and then switch back to your session model for execution. Use `/model plan` (or `/model --plan`) to select a plan-mode model:
...
This lets you use a more capable model (like Claude Opus 5) for nuanced planning while using a faster model for code generation.
