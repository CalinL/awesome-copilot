...
If you're working in the Copilot CLI and want to jump into the GitHub Copilot app for your current directory, use:

```bash
copilot app
```

This opens the Copilot app focused on the repository in your current working directory — a quick way to switch from CLI to the app's visual interface without navigating manually.
