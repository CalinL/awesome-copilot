...
If you're already working in a Copilot CLI session and want to move it to the desktop app, use the `/app` command:

```bash
/app
```

This opens the current CLI session in the GitHub Copilot desktop app, picking up exactly where you left off. The app will display the session's timeline and let you continue steering it from the GUI.
